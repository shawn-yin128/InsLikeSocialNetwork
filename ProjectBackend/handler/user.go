...
func signinHandler(w http.ResponseWriter, r *http.Request) {
	...
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{ // MapClaims is payload
		"username": user.Username,
		"exp":      time.Now().Add(time.Hour * 24).Unix(),
	})

	tokenString, err := token.SignedString(mySigningKey) // given secret generate token
	if err != nil {
